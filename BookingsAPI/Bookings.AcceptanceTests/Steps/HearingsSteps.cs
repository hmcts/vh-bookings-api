...
namespace Bookings.AcceptanceTests.Steps
{
    ...
    public sealed class HearingsSteps
    {
        ...
        public void ThenHearingDetailsShouldBeRetrievedForTheCaseType()
        {
            ...
            var response = model.Hearings.SelectMany(u => u.Hearings).FirstOrDefault(x => x.ScheduledDateTime.Date == DateTime.UtcNow.AddDays(2).Date && x.HearingId == _acTestContext.HearingId);

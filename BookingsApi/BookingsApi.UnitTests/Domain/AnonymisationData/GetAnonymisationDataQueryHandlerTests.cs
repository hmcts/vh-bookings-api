...
namespace BookingsApi.UnitTests.Domain.AnonymisationData
{
    public class GetAnonymisationDataQueryHandlerTests
    {
        ...
        public async Task Setup()
        {
            ...
            _person3 = new Person(Faker.Name.Suffix(), Faker.Name.First(), Faker.Name.Last(), Faker.Internet.Email()) { ContactEmail = Faker.Internet.Email() };
            _person4 = new Person(Faker.Name.Suffix(), Faker.Name.First(), Faker.Name.Last(), Faker.Internet.Email()) { ContactEmail = Faker.Internet.Email() };
            _anonymisedPerson = new Person(Faker.Name.Suffix(), Faker.Name.First(), Faker.Name.Last(),  $"{Faker.Internet.Email()}@email.net") { ContactEmail = Faker.Internet.Email() };
            
            _caseType1 = new CaseType(12, "case 1") {HearingTypes = new List<HearingType> {new HearingType("Hearing type 1")}};
        ...
        public async Task GetAnonymisationDataQuery_Filters_Out_Usernames_And_Hearing_Ids_Already_Processed_By_LastRunDate_Specified_In_JobHistory()
        {
            ...
            var videoHearingType = typeof(VideoHearing);
            videoHearingType.GetProperty("ScheduledDateTime").SetValue(hearing1, DateTime.UtcNow.AddMonths(-10)); //setting the scheduled time for a hearing that does not fall in between cut off date and last run date 

...
public class HearingParticipantService : IHearingParticipantService
{
    ...
    public async Task PublishEventForNewParticipantsAsync(VideoHearing hearing, IEnumerable<NewParticipant> newParticipants)
    {
        ...
        if (participants.Any())
        {
            var participantsIds = participants.Select(x => x.Id).ToList();
            ...
            await _participantAddedToHearingAsynchronousProcess.Start(hearing, participantsIds);
        }
    }
        ...
        List<Participant> eventExistingParticipants, List<Participant> eventNewParticipants, bool sendNotification = true)
    {
        ...
        if (sendNotification)
        {
            var eventNewParticipantIds = eventNewParticipants.Select(p => p.Id).ToList();
            await _participantAddedToHearingAsynchronousProcess.Start(hearing, eventNewParticipantIds);
        }
    }

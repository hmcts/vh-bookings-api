...
public static class ParticipantRequestToNewParticipantMapper
{
    ...
    {
        var caseRole = caseType.CaseRoles.Find(x => x.Name.Equals(requestParticipant.CaseRoleName, StringComparison.InvariantCultureIgnoreCase));
        if (caseRole == null) throw new BadRequestException($"Invalid case role [{requestParticipant.CaseRoleName}]");

        var hearingRole = caseRole.HearingRoles.Find(x => x.Name.Equals(requestParticipant.HearingRoleName, StringComparison.InvariantCultureIgnoreCase));

...
public class HearingAllocationService(
    ...
    : IHearingAllocationService
{
    private readonly AllocateHearingConfiguration _configuration = configuration.Value;
    ...
    public async Task<JusticeUser> AllocateManually(Guid hearingId, Guid justiceUserCsoId)
    {
        ...
        {
            // we need to un-allocate the hearing and allocate to the new user
            hearing.Deallocate();
        }
    ...
    {
        var user = await context.JusticeUsers
    ...
    {
        var csoIndex = randomNumberGenerator.Generate(1, csos.Count);
